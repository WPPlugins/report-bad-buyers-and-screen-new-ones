...
class Ebuyers_Reviewed {
	...
	private function define_api_key() {
		$settings = get_option('ebuyers_reviewed_settings');
		if(!isset($settings['ebuyers_api_key']) || $settings['ebuyers_api_key'] == '') {
			$this->loader->add_action( 'admin_notices', $this, 'give_api_key_notice' );
		} else {
			$this->api_key = $settings['ebuyers_api_key'];
		}
	}
  ...
  public function give_api_key_notice() { 
    ...
    <div class="notice notice-error is-dismissible">
    	<p>You need to add your API key For eBuyersReviewed. You can do that <a href="<?php echo get_admin_url(); ?>/admin.php?page=ebuyers-reviewed-settings">here</a></p>
    </div>
    <?php
  }
